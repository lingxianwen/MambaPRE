...
run_one() {
  local tag="$1"
  local base="$2"
  local config="runs/configs/candidate_${tag}_seed${seed}.json"
  local run_dir="runs/controlled_candidate_selection/pw8/${tag}_seed${seed}"
  local result_dir="results/controlled_candidate_selection_pw8/seed${seed}/${tag}"
  mkdir -p "$result_dir"
  "$python_bin" scripts/make_candidate_selection_config.py --base "$base" --seed "$seed" --tag "pw8/${tag}" --output "$config"
